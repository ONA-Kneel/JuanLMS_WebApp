...
router.get('/export', authenticateToken, async (req, res) => {
  try {
    ...
    const dataRowCount = registrants.length;
    const footerRow = dataRowCount + 10; // Adjust based on header rows
    worksheet.addRow([]); // Empty row
    worksheet.addRow([
...
// GET /api/registrants/export-pdf?date=YYYY-MM-DD&status=pending
router.get('/export-pdf', authenticateToken, async (req, res) => {
  try {
    ...
    // Fetch registrants with filters
    ...
    console.error('Server error in export-pdf endpoint:', err);
    res.status(500).json({ message: 'Failed to export registrants to PDF', error: err.message });
  }
});
