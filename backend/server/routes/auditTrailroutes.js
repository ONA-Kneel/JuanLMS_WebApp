...
router.get('/audit-logs/export-pdf', authenticateToken, async (req, res) => {
  try {
    ...
    doc.fontSize(24).font('Helvetica-Bold').text('Audit Logs Export', { align: 'center' });
    doc.moveDown(0.5);
    
    // Add subtitle
    doc.fontSize(14).font('Helvetica').text(`Generated on ${dateStr} at ${timeStr}`, { align: 'center' });
    doc.moveDown(0.5);
    
    // Add summary
    doc.fontSize(12).font('Helvetica').text(`Total Logs: ${logs.length}`, { align: 'center' });
    doc.fontSize(12).font('Helvetica').text(`Filters: Action=${action || 'All'}, Role=${role || 'All'}`, { align: 'center' });
    doc.moveDown(1.5);
    ...
    // Add footer
    doc.moveDown(2);
    doc.fontSize(10).font('Helvetica').text(`Report generated by JuanLMS System`, { align: 'center' });
    doc.fontSize(10).font('Helvetica').text(`Page ${doc.bufferedPageRange().count}`, { align: 'center' });
