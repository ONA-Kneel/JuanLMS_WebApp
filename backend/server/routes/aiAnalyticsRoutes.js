...
router.post("/create-analysis", authenticateToken, async (req, res) => {
  try {
    ...
    if (inferredReportType === 'year') {
      focusBlock = `REPORT SCOPE: Whole active academic year (no specific strand/section filter).\nOUTPUT RULES: Only summarize school-year-wide findings and recommendations. Do NOT include strand- or section-specific analyses or headings. Avoid placeholder sections.`;
    } else if (inferredReportType === 'strand') {
      focusBlock = `REPORT SCOPE: Specific STRAND${strandFilter ? ` — ${strandFilter}` : ''}.\nOUTPUT RULES: Only discuss this strand. Do NOT include school-year-wide or section-specific summaries. Do NOT include generic headings like "By Section/Track/Strand" unless they refer to this strand only.`;
    } else if (inferredReportType === 'section') {
      focusBlock = `REPORT SCOPE: Specific SECTION${sectionFilter ? ` — ${sectionFilter}` : ''}.\nOUTPUT RULES: Only discuss this section. Include: how students are doing, strengths, weaknesses, and targeted interventions. Do NOT include school-year-wide or strand-wide content. Avoid generic directory or system-wide headings.`;
    }
    ...
    const prompt = `As an educational analytics expert, prepare a role-aware report.\n${audienceLine}\n${focusBlock}\n\nSCHOOL YEAR: ${schoolYear}\nTERM: ${termName}\n\nDATA SUMMARY (scoped):\n- Total faculty assignments (scoped): ${analysisData.facultyAssignments}\n- Sections (scoped): ${analysisData.sections.join(', ') || 'None'}\n- Tracks (scoped): ${analysisData.tracks.join(', ') || 'None'}\n- Strands (scoped): ${analysisData.strands.join(', ') || 'None'}\n- Faculty (scoped): ${analysisData.facultyNames.join(', ') || 'None'}\n- Workload (scoped): ${workloadLines || '- None'}\n\nACTIVITY SUMMARY (scoped):\n- Total: ${analysisData.activitySummary.totalActivities}\n- Assignments: ${analysisData.activitySummary.assignmentsCount}\n- Quizzes: ${analysisData.activitySummary.quizzesCount}\n- Posted: ${analysisData.activitySummary.postedActivities}\n- Pending: ${analysisData.activitySummary.pendingActivities}\n\nSTUDENT ENGAGEMENT (scoped):\n- Interactions total: ${analysisData.studentEngagement.totalStudents}\n- Logins: ${analysisData.studentEngagement.loginCount}\n- Logouts: ${analysisData.studentEngagement.logoutCount}\n- Other actions: ${analysisData.studentEngagement.otherActions}\n\nSTRICT OUTPUT CONSTRAINTS:\n- Do NOT include unrelated sections such as generic "By Section/Track/Strand" if outside the scope.\n- Do NOT invent directories or placeholders.\n- Keep the report strictly within the chosen scope (${inferredReportType}).\n\nDELIVERABLES:\n1) Executive summary for the chosen scope.\n2) Key findings for ${inferredReportType}.\n3) If STRAND: what is seen in the strand + concrete improvements.\n4) If SECTION: student performance status, strengths, weaknesses, and targeted interventions.\n5) Actionable recommendations and next steps.\nFormat clearly for leadership review.`;
