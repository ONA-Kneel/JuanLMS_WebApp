    ...
    try {
      ...
      const matchingChats = unifiedChats.filter(chat => {
        if (chat.type === 'individual') {
          const contactNickname = contactNicknames[chat._id] || null;
          return (
            getUserDisplayName(chat, contactNickname)?.toLowerCase().includes(searchValue.toLowerCase()) ||
            (chat.nickname && chat.nickname.toLowerCase().includes(searchValue.toLowerCase()))
          );
        } else {
      ...
      const localSearchResults = searchValue.trim() === '' ? [] : [
        ...
        ...users
          ...
          .filter(user => !recentChats.some(chat => chat._id === user._id))
          .filter(user =>
            getUserDisplayName(user, contactNicknames[user._id] || null)?.toLowerCase().includes(searchValue.toLowerCase()) ||
