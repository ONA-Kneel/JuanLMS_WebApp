...
const API_BASE = import.meta.env.VITE_API_URL || "https://juanlms-webapp-server.onrender.comp-server.onrender.comp-server.onrender.comp-server.onrender.com";
...
const Faculty_Meeting = () => {
  ...
  console.log('[DEBUG] handleJoinMeeting received:', meeting);
    ...
    const token = localStorage.getItem('token');
    const response = await fetch(`${import.meta.env.VITE_API_URL || "https://juanlms-webapp-server.onrender.comp-server.onrender.comp-server.onrender.comp-server.onrender.com"}/api/meetings/${meeting._id}/join`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token}` }
    });
