    ...
    */
    ...
    export default function Faculty_Grades() {
      ...
      const [quarterlyExamHPS, setQuarterlyExamHPS] = useState(100); // Class-wide HPS for quarterly exam
      ...
      }, [selectedClass, selectedSection, selectedQuarter, classes]); // grades removed to prevent infinite loop
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (savedGradesData.success && savedGradesData.data && savedGradesData.data.grades) {
              ...
              // Load the class-wide HPS value
              if (savedGradesData.data.quarterlyExamHPS) {
                setQuarterlyExamHPS(savedGradesData.data.quarterlyExamHPS);
              }
            }
          }
        } catch (error) {
      ...
      useEffect(() => {
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [selectedClass, selectedSection, students, selectedQuarter, loadSavedGrades]); // loadQuarterlyGradesFromDatabase removed to prevent infinite loop
      ...
      const handleGradeChange = (studentId, gradeType, value) => {
        ...
        // If it's not a valid number, keep the original value for user to continue typing
        ...
        if (gradeType === 'quarterlyExam') {
          // Use the class-wide HPS value
          if (numericValue > quarterlyExamHPS) {
            validatedValue = quarterlyExamHPS.toString();
        ...
        setGrades(prev => ({
          ...
          [studentId]: {
            ...prev[studentId],
            [gradeType]: validatedValue
          }
        }));
      };

      // Direct quarterly exam change handler
      const handleQuarterlyExamChange = (studentId, value) => {
        handleGradeChange(studentId, 'quarterlyExam', value);
      };
      ...
      const handleQuarterlyHPSChange = (e) => {
        const raw = e.target.value;

        // allow the user to clear temporarily (so they can type)
        if (raw === '' || raw === null) {
          setQuarterlyExamHPS('');
          return;
        }
        ...
        const n = Number(raw);
        if (Number.isNaN(n)) return; // ignore invalid characters

        // clamp to 0..100
        const clamped = Math.max(0, Math.min(100, n));
        setQuarterlyExamHPS(clamped);
      };

      const normalizeQuarterlyHPSOnBlur = () => {
        // if empty, default back to 100; otherwise re-clamp
        const n = Number(quarterlyExamHPS);
        if (Number.isNaN(n)) {
          setQuarterlyExamHPS(100);
          return;
        }
        const clamped = Math.max(0, Math.min(100, n));
        setQuarterlyExamHPS(clamped);
      ...
      const saveGrades = async () => {
        ...
        try {
          ...
          const gradesData = {
            ...
            quarterlyExamHPS: quarterlyExamHPS, // Save the class-wide HPS value
      ...
      const calculateFinalGrade = (studentId, className, quarterlyScore = 0) => {
        ...
        // Normalize quarterly exam raw score by the class-wide HPS to get PS%, then apply weight to get WS
        const quarterlyPS = quarterlyExamHPS > 0 ? (quarterlyScore / quarterlyExamHPS) * 100 : 0;
        const quarterlyWS = quarterlyPS * quarterly / 100;
        const rawFinalGrade = initialGrade + quarterlyWS;
                          ...
                          <th className="border border-gray-300 px-2 py-2 text-center font-semibold">
                            <div className="flex flex-col items-center">
                              ...
                              <input
                                ...
                                max="100"
                                step="0.01"
                                className="w-16 text-center border border-gray-300 rounded px-1 py-1 mt-1 text-xs"
                                value={quarterlyExamHPS}
                                onChange={handleQuarterlyHPSChange}
                                onBlur={normalizeQuarterlyHPSOnBlur}
                                placeholder="100"
                                inputMode="decimal"
                              />
                      ...
                      <tbody>
                        {students.map((student) => (
                          <tr key={student._id} className="hover:bg-gray-50">
                            ...
                            <td className="border border-gray-300 px-2 py-2">
                              <input
                                key={`quarterly-exam-${student._id}-${selectedQuarter}`}
                                type="number"
                                min="0"
                                max={quarterlyExamHPS}
                                step="0.01"
                                className="w-full text-center border-none focus:outline-none focus:ring-1 focus:ring-blue-500 bg-transparent"
                                value={grades[student._id]?.quarterlyExam || ''}
                                onChange={(e) => handleQuarterlyExamChange(student._id, e.target.value)}
                                onBlur={(e) => {
                                  // Ensure the value is properly formatted on blur
                                  const value = e.target.value;
                                  if (value && !isNaN(parseFloat(value))) {
                                    const numValue = parseFloat(value);
                                    if (numValue > quarterlyExamHPS) {
                                      handleGradeChange(student._id, 'quarterlyExam', quarterlyExamHPS.toString());
                                    } else if (numValue < 0) {
                                      handleGradeChange(student._id, 'quarterlyExam', '0');
                                    }
                                  }
                                }}
                                placeholder="0"
