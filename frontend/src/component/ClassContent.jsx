...
export default function ClassContent({ selected, isFaculty = false }) {
  ...
  useEffect(() => {
    ...
    fetch('https://juanlms-webapp-server.onrender.com/classes', {
  ...
  useEffect(() => {
    if (selected === "members") {
      ...
      fetch(`https://juanlms-webapp-server.onrender.com/classes/${classId}/members`)
        .then(res => res.json())
        .then(data => {
  ...
  useEffect(() => {
    if (selected === "classwork" && realClassId) {
      ...
      const token = localStorage.getItem('token');
      fetch(`https://juanlms-webapp-server.onrender.com/assignments?classID=${realClassId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
        .then(res => res.json())
  ...
  useEffect(() => {
    ...
    if (userRole === 'faculty') {
      ...
      Promise.all([
        fetch(`https://juanlms-webapp-server.onrender.com/assignments/${selectedAssignment._id}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        }).then(res => res.json()),
        fetch(`https://juanlms-webapp-server.onrender.com/classes/${classId}/members`).then(res => res.json())
      ...
      fetch(`https://juanlms-webapp-server.onrender.com/assignments/${selectedAssignment._id}/view`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
    }
  ...
  const handleAddLesson = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch("https://juanlms-webapp-server.onrender.com/lessons", {
        method: "POST",
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const handleDeleteLesson = async (lessonId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`https://juanlms-webapp-server.onrender.com/lessons/${lessonId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleDeleteFile = async (lessonId, fileUrl) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`https://juanlms-webapp-server.onrender.com/lessons/${lessonId}/file?fileUrl=${encodeURIComponent(fileUrl)}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleDeleteAssignment = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`https://juanlms-webapp-server.onrender.com/assignments/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
